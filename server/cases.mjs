...
export function caseRow(id) {
  ...
  if (!row) return null;
  return { ...row, insured: Boolean(row.insured), major: Boolean(row.major), criminalRisk: Boolean(row.criminalRisk), escalated: Boolean(row.escalated), isDemo: Boolean(row.isDemo), clarificationAnswers: JSON.parse(row.clarificationAnswers) };
}
...
export async function getAnalysis(item, evidence = evidenceRows(item.id)) {
  return analyzeCase(item, evidence, knowledgeRows());
}
...
export function saveAnalysis(item, analysis, user) {
  ...
  if ((analysis.escalationReasons || []).length && !item.escalated && item.status !== '已归档') {
    run('UPDATE "Case" SET escalated=1,status=? WHERE id=?', '法务处理中', item.id);
    const legal = one('SELECT id FROM User WHERE role=? ORDER BY id LIMIT 1', 'legal');
    addTask(item, { title: '人工法务审核：' + analysis.escalationReasons.join('；'), kind: 'legal_review', priority: 'P0', dueAt: new Date(Date.now() + 4 * 3600000).toISOString(), assignedTo: legal.id });
    audit(user, '自动升级法务', analysis.escalationReasons.join('；') + '。仅作分流，最终决定由人工法务作出。', item.id);
  }
