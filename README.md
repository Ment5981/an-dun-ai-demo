...
没有配置模型密钥时，系统使用服务端规则和本地中文检索，界面显示“规则辅助模式”。配置兼容 OpenAI Chat Completions 的服务地址、`AI_API_KEY` 和 `AI_MODEL` 后，模型只会基于检索到的候选来源辅助整理摘要、焦点和问题；风险升级、证据要求、引用白名单和法务锁仍由服务端约束。当前 MVP 不做 OCR 或视频内容识别，上传原件后需要人工核验内容。
