...
export function AgentChat({ role, onCreateCase, onClose }: { role: Role; onCreateCase?: (draft: AgentDraft) => void; onClose?: () => void }) {
  ...
  const voice = useRef<SpeechRecognitionLike | null>(null);
  ...
  const toggleVoice = () => {
    ...
    const recognition = new Ctor(); voice.current = recognition; recognition.lang = 'zh-CN'; recognition.continuous = false; recognition.interimResults = false;
    ...
    recognition.onend = () => setListening(false); recognition.onerror = () => { setListening(false); setError('没有识别到清晰语音，请再试一次。'); }; setError(''); try { recognition.start(); setListening(true); } catch { setError('无法启动麦克风，请检查浏览器权限。'); setListening(false); }
  };
  ...
  return <section className={`agent-chat-panel ${messages.length === 1 ? 'chat-empty' : 'chat-active'}`} aria-label="AI 案件助手"><div className="agent-chat-header"><div className="agent-chat-brand"><span><Sparkle24Regular /></span><div><strong>AI 案件助手</strong><small>先聊天，再自动整理成案件</small></div></div>{onClose && <Button appearance="subtle" icon={<Dismiss24Regular />} aria-label="关闭 AI 案件助手" onClick={onClose}/>}</div><div className="agent-chat-intro"><ShieldTask24Regular/><p>不用先填表。说清楚“发生了什么”，AI 会继续追问时间、货物、客户诉求和手上已有的证据。</p></div><div className="agent-chat-messages" aria-live="polite">{messages.map((message, index) => <div className={`agent-message ${message.role}`} key={`${message.role}-${index}`}><span className="agent-message-role">{message.role === 'assistant' ? 'AI 助手' : '我'}</span><p>{message.content}</p></div>)}{busy && <div className="agent-message assistant"><span className="agent-message-role">AI 助手</span><p className="agent-typing">正在理解案情<span>·</span><span>·</span><span>·</span></p></div>}<div ref={end}/></div><div className="agent-quick-prompts">{quick.map(prompt => <button key={prompt} type="button" onClick={() => setInput(prompt)}>{prompt}</button>)}</div>{lastResult?.draft && <div className="agent-draft-card"><div><span className="agent-draft-label">已整理案件草稿 · {lastResult.mode === 'llm' ? lastResult.model || '模型增强' : '本地规则'}</span><strong>{lastResult.draft.title}</strong><p>{lastResult.draft.category} · {lastResult.draft.amount ? `主张金额 ¥${lastResult.draft.amount.toLocaleString('zh-CN')}` : '金额待补充'}{lastResult.draft.goods ? ` · ${lastResult.draft.goods}` : ''}</p></div>{onCreateCase && <Button appearance="primary" icon={<ArrowRight24Regular />} iconPosition="after" onClick={() => onCreateCase(lastResult.draft!)}>用对话创建案件</Button>}</div>}{error && <div className="error-box" role="alert">{error}</div>}<form className="agent-chat-compose" onSubmit={send}><textarea aria-label="告诉 AI 发生了什么" value={input} onChange={event => setInput(event.target.value)} onKeyDown={event => { if(event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} placeholder="例如：客户收到相机后发现镜头破损，要求赔偿……" rows={3} maxLength={6000} disabled={busy}/><Button type="button" appearance={listening ? 'primary' : 'secondary'} className="agent-voice-button" icon={<Mic24Regular />} onClick={toggleVoice}>{listening ? '正在听…' : '语音输入'}</Button><Button appearance="primary" type="submit" icon={<Send24Regular />} disabled={!input.trim() || busy}>发送</Button></form><small className="agent-chat-disclaimer">{lastResult?.disclaimer || 'AI 仅辅助整理事实和证据，不替代主管或法务决策。当前身份：' + roleNames[role]}</small></section>;
}

export function AgentPage({ role, onCreateCase }: { role: Role; onCreateCase: (draft: AgentDraft) => void }) {
  return <div className="agent-page"><div className="agent-page-heading"><div><span className="agent-page-kicker"><Sparkle24Regular/> 案盾 · 你的纠纷处理助手</span><h1>你好，今天需要处理什么？</h1><p>从一句话开始，把复杂的纠纷理清楚。</p></div><div className="agent-page-tip"><Mic24Regular/><span>支持中文语音输入<br/><small>浏览器首次使用时请允许麦克风</small></span></div></div><AgentChat role={role} onCreateCase={onCreateCase}/><div className="agent-page-foot"><span>确认草稿后，案情将带入建案表单</span><span>AI 只做辅助研判，责任和赔偿结论由授权人员确认</span></div></div>;
}
